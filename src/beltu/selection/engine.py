...
DEFAULT_PROFILES: tuple[CapabilityProfile, ...] = (
    ...
    CapabilityProfile(
        "browser.automation",
        "Run bounded browser UI workflows without arbitrary JavaScript execution.",
        0.70, 0.75, "medium", True,
        required_kinds=frozenset({"endpoint", "login", "api_endpoint"}),
        produced_kinds=frozenset({"browser.navigation", "browser.response", "browser.extraction"}),
        external=True,
        preferred_tools=("browser",),
    ),
    CapabilityProfile(
        "http.workflow",
        "Replay bounded in-scope HTTP sequences with variables and assertions.",
        0.54, 0.79, "medium", True,
        required_kinds=frozenset({"endpoint", "api_endpoint", "web.http_probe"}),
        produced_kinds=frozenset({"http.response", "workflow.execution"}),
        external=True,
        preferred_tools=("http-workflow",),
    ),
    CapabilityProfile(
        "session.replay",
        "Replay a bounded workflow using an explicit local session profile.",
        0.62, 0.82, "high", True,
        required_kinds=frozenset({"session", "auth", "login"}),
        produced_kinds=frozenset({"session.replay", "http.response"}),
        external=True,
        preferred_tools=("session-replay",),
    ),
    CapabilityProfile(
        "api.manipulation",
        "Apply a bounded set of API parameter mutations to an identified operation.",
        0.67, 0.81, "high", True,
        required_kinds=frozenset({"api_endpoint", "api", "openapi"}),
        produced_kinds=frozenset({"api.manipulation_result"}),
        external=True,
        preferred_tools=("api-manipulation",),
    ),
    CapabilityProfile(
        "authorization.interactive",
        "Compare explicitly supplied principals against the same in-scope operation.",
        0.82, 0.86, "high", True,
        required_kinds=frozenset({"authorization", "access_control", "role"}),
        produced_kinds=frozenset({"authorization.response", "authorization.comparison"}),
        external=True,
        preferred_tools=("authorization-matrix",),
    ),
    CapabilityProfile(
        "business_logic.workflow",
        "Execute a bounded state-aware workflow with per-step assertions.",
        0.84, 0.86, "high", True,
        required_kinds=frozenset({"workflow", "state_transition", "order", "cart"}),
        produced_kinds=frozenset({"workflow.execution", "workflow.assertion_failed"}),
        external=True,
        preferred_tools=("business-logic",),
    ),
    CapabilityProfile(
        "race_condition.test",
        "Run a tightly bounded concurrent request test against one operation.",
        0.92, 0.83, "high", True,
        required_kinds=frozenset({"api_endpoint", "workflow", "order", "cart"}),
        produced_kinds=frozenset({"race.response", "race.summary"}),
        external=True,
        preferred_tools=("race-condition",),
    ),
    CapabilityProfile(

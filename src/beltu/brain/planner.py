        ...
        for hypothesis in hypotheses[:3]:
            ...
            if "not characterized" in text:
                ...
                actions.append(ActionProposal(
                    ...
                    {"target": context.target, "hypothesis": hypothesis.statement},
                    "Test a bounded concurrent request only when workflow evidence suggests a state/rate-limit race.",
                    hypothesis.confidence,
                    "high",
                    True,
                ))
            ...
            elif "parameter" in text or "api" in text or "tamper" in text:
                actions.append(ActionProposal(
                    "api_manipulation",
                    {"target": context.target, "hypothesis": hypothesis.statement},
                ...
                actions.append(ActionProposal(
                    "browser_automation",
                    {"target": context.target, "hypothesis": hypothesis.statement},
                    "Use the bounded browser worker to exercise UI state and capture network metadata.",
                    hypothesis.confidence,
                    "medium",
                    True,
                ))
            elif "request" in text or "replay" in text or "http" in text:
                actions.append(ActionProposal(
                    "http_workflow",
                    {"target": context.target, "hypothesis": hypothesis.statement},
